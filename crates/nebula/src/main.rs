...
//! バックエンド接続・言語文法のコンパイル・git 状態の取得・言語サーバーの起動は
//! すべてフレーム後に回す。この境界を越えて良いのは定数の構築とウィンドウ生成
//! だけで、I/O を伴うものは理由を問わず後回しにする。
//!
//! Metal シェーダはビルド時にプリコンパイルする (gpui の `runtime_shaders`
//! フィーチャは使わない)。起動時にコンパイルすると数十 ms かかり、冷間起動の
//! 予算をそれだけで圧迫するため。
//!
