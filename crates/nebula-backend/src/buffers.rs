...
//!
//! バッファの正本はここにしか無い。GUI が持つのは編集を即座に描くための楽観的な
//! 複製で、キー入力のたびに [`apply_edits`] へ `base_version` を添えて送り、
//! ここでの適用結果 (新しい版数) と突き合わせて自分の複製を正しいと確認する。
//! `base_version` が食い違えば拒否し、GUI に正本の取り直しを促す。
//!
//! [`apply_edits`] は要求ごとに別タスクで処理されるため、GUI が複数の編集要求を
//! 並行して送ると適用順が入れ替わり版数がずれる。呼び出し側は `ApplyEdits` を
//! 応答が返るまで次を送らない、1 件ずつの直列送信を守る必要がある。保存
//! ([`save`]) も同じ待ち行列に載せるべき要求で、独立に投げるとまだ送っていない
//! 編集を追い越してディスクに古い内容が書かれる。
