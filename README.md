...
- GUI はバッファの複製を持ち楽観的に描く。正本はバックエンドで、版数が食い違えば取り直す
- 構文解析は GUI では走らせない (バックエンドが可視行のスパンだけを返す)
- IPC は 4 バイト長 + MessagePack の 1 本のストリーム。Unix はドメインソケット、Windows は名前付きパイプ
- バックエンドはユーザーあたり 1 プロセス。排他は待ち受けそのものに任せる
- 外部ツールは「あれば使う」。無ければ該当機能だけが無効になる
- 最初のフレームより前に置くのはテーマ・キーバインド・フォントの登録とウィンドウ生成だけ
